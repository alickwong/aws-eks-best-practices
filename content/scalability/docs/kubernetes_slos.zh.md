...
|API请求延迟(变更)	|处理单个对象的变更API调用的延迟,以过去5分钟的99百分位数衡量	|在默认的Kubernetes安装中,对于每个(资源,动词)对,不包括虚拟和聚合资源以及自定义资源定义,每个集群每天的99百分位数 <= 1秒	|
|API请求延迟(只读)	|处理非流式只读API调用的延迟,以每个(资源,范围)对的99百分位数衡量	|在默认的Kubernetes安装中,对于每个(资源,范围)对,不包括虚拟和聚合资源以及自定义资源定义,(a) 如果`scope=resource`,99百分位数每集群每天 <= 1秒 (b) 否则(如果`scope=namespace`或`scope=cluster`),99百分位数每集群每天 <= 30秒	|
|Pod启动延迟	|可调度无状态Pod的启动延迟,不包括拉取镜像和运行init容器的时间,从Pod创建时间戳到所有容器报告为已启动并通过watch观察到的时间,以过去5分钟的99百分位数衡量	|在默认的Kubernetes安装中,每个集群每天的99百分位数 <= 5秒	|

### API请求延迟

`kube-apiserver`默认将`--request-timeout`定义为`1m0s`,这意味着请求最多可以运行一分钟(60秒)才会超时并被取消。定义的SLO延迟指标根据请求的类型(变更或只读)进行细分:
...
这种延迟以过去5分钟的99百分位数来衡量Kubernetes资源的(资源,动词)对,例如测量创建Pod请求和更新Node请求的延迟。请求延迟必须 <= 1秒才能满足SLO。

#### 只读

只读请求会检索单个资源(如获取Pod X)或集合(如"从命名空间X获取所有Pod")。`kube-apiserver`维护了一个对象缓存,所以请求的资源可能来自缓存,也可能需要先从etcd中检索。
这些延迟也是以过去5分钟的99百分位数来衡量,但只读请求可以有不同的范围。SLO定义了两个不同的目标:

* 对于请求单个资源(即`kubectl get pod -n mynamespace my-controller-xxx`)的情况,请求延迟应保持在 <= 1秒。
* 对于请求命名空间或集群中的多个资源(例如`kubectl get pods -A`)的情况,延迟应保持在 <= 30秒

SLO 对于不同的请求范围有不同的目标值,因为针对 Kubernetes 资源列表的请求需要在 SLO 内返回所有对象的详细信息。在大型集群或大量资源集合中,这可能导致响应大小较大,需要一些时间才能返回。例如,在运行数万个 Pod 的集群中,每个 Pod 在 JSON 编码时大约为 1 KiB,返回集群中的所有 Pod 将包含 10MB 或更多。Kubernetes 客户端可以通过[使用 APIListChunking 分块检索大型资源集合](https://kubernetes.io/docs/reference/using-api/api-concepts/#retrieving-large-results-sets-in-chunks)来减小响应大小。

### Pod 启动延迟

此 SLO 主要关注从 Pod 创建到容器实际开始执行的时间。为了测量这一点,计算从记录在 Pod 上的创建时间戳到[监视该 Pod](https://kubernetes.io/docs/reference/using-api/api-concepts/#efficient-detection-of-changes)报告容器已启动的时间差(不包括容器镜像拉取和 init 容器执行的时间)。为了满足 SLO,每个集群日的 Pod 启动延迟的 99 百分位必须保持在 <=5 秒以内。

请注意,此 SLO 假设工作节点已经处于就绪状态,以便调度 Pod。此 SLO 不考虑镜像拉取或 init 容器执行,并且还将测试限制为不使用持久存储插件的"无状态 Pod"。
...
|指标	|定义	|
|---	|---	|
|apiserver_request_sli_duration_seconds	| 每个动词、组、版本、资源、子资源、范围和组件的响应延迟分布(不包括 webhook 持续时间和优先级和公平性队列等待时间)，单位为秒。	|
|apiserver_request_duration_seconds	| 每个动词、干运行值、组、版本、资源、子资源、范围和组件的响应延迟分布,单位为秒。	|

*注意: `apiserver_request_sli_duration_seconds` 指标从 Kubernetes 1.27 开始可用。*

您可以使用这些指标来调查 API 服务器响应时间,以及 Kubernetes 组件或其他插件/组件中是否存在瓶颈。以下查询基于[社区 SLO 仪表板](https://github.com/kubernetes/perf-tests/tree/master/clusterloader2/pkg/prometheus/manifests/dashboards)。
...
在这些查询中,我们排除了不会立即返回的流 API 请求,如 `kubectl port-forward` 或 `kubectl exec` 请求(`subresource!~"proxy|attach|log|exec|portforward"`)。我们只过滤修改对象的 Kubernetes 动词(`verb=~"CREATE|DELETE|PATCH|POST|PUT"`)。然后我们计算过去 5 分钟内该延迟的第 99 百分位。
...
对于同一命名空间内的资源集合(scope=namespace, threshold=5s)
...
SLI指标提供了Kubernetes组件的性能洞察,排除了请求在API优先级和公平性队列中等待、通过准入Webhook或其他Kubernetes扩展处理的时间。总体指标提供了更全面的视角,因为它反映了应用程序等待API服务器响应的时间。比较这些指标可以提供对请求处理延迟的洞察。

### Pod启动延迟

|指标	| 定义	|
|---	|---	|
|kubelet_pod_start_sli_duration_seconds	|启动Pod所需的时间,不包括拉取镜像和运行Init容器的时间,从Pod创建时间戳到所有容器报告为已启动并通过watch观察到的时间	|
|kubelet_pod_start_duration_seconds	|从Kubelet第一次看到Pod到Pod开始运行所需的时间。这不包括调度Pod或扩展工作节点容量的时间。	|

*注意:  `kubelet_pod_start_sli_duration_seconds`从Kubernetes 1.27开始可用。*

类似于上述查询,您可以使用这些指标来了解节点扩展、镜像拉取和Init容器对Pod启动延迟的影响。

**Pod启动延迟SLI -** 这是从Pod创建到应用程序容器报告为运行的时间。这包括工作节点容量可用和Pod调度的时间,但不包括拉取镜像或运行Init容器的时间。  
`histogram_quantile(0.99, sum(rate(kubelet_pod_start_sli_duration_seconds_bucket[5m])) by (le))`

**Pod启动延迟总计 -** 这是Kubelet第一次启动Pod所需的时间。这是从Kubelet通过WATCH接收Pod开始计算的,不包括工作节点扩展或调度的时间。这包括拉取镜像和运行Init容器的时间。  
...
如果您正在使用 `kube-prometheus-stack` 或类似的 Prometheus 解决方案,您可以安装相同的仪表板来实时观察集群上的 SLO。
...
请注意,SLO 侧重于集群中 Kubernetes 组件的性能,但您还可以查看其他指标,这些指标提供了不同的视角或洞见。Kubernetes 社区项目如 [Kube-state-metrics](https://github.com/kubernetes/kube-state-metrics/tree/main) 可以帮助您快速分析集群趋势。来自 Kubernetes 社区的大多数常见插件和驱动程序也会发出 Prometheus 指标,允许您调查自动扩缩器或自定义调度程序等内容。
